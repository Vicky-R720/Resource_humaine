...
public interface SalaryParametersRhRepository extends JpaRepository<SalaryParametersRh, Long> {
        ...
        @Query("""
                            SELECT s FROM SalaryParametersRh s
                            WHERE s.nomParametre = :nomParam
                            AND s.isActive = true
                            AND (
                                    (:date BETWEEN s.dateDebutValidite AND s.dateFinValidite)
                                    OR (s.dateFinValidite IS NULL AND s.dateDebutValidite <= :date)
                                )
                        """)
        Optional<SalaryParametersRh> findActiveByNomParametre(
                        @Param("nomParam") String nomParam,
                        @Param("date") LocalDate date);
