...
public class PersonController {
    ...
    public ResponseEntity<List<Person>> getAll() {
        try {
            List<Person> persons = personService.getAllPersonsNotLinkedWithPersonnel();
            return ResponseEntity.ok(persons); // ✔ OK
        } catch (Exception e) {
    ...
    public ResponseEntity<Person> updatePerson(
        ...
        person.setId(id);
        Person updated = personService.createPerson(person);
        ...
        if (person.getAppliances() != null) {
            for (int i = 0; i < person.getAppliances().size(); i++) {
                ...
                if (candidature.getOffer() != null) {
                    ...
                    detailsCandidature.put("posteOffre",
                            offre.getPost() != null ? offre.getPost().getName() : "Non spécifié");
    ...
    public ResponseEntity<Map<String, Object>> acceptAppliance(@PathVariable Long id) {
        try {
            ...
            notificationService.createNotification(
                    ...
                    appliance);
            personnelService
                    .updatePersonnel(new Personnel(appliance.getPerson(), appliance.getOffer().getPost().getName()));
                ...
                .filter(appliance -> {
                    ...
                    if (offer != null && !offer.trim().isEmpty()) {
                        String postName = appliance.getOffer() != null &&
                                appliance.getOffer().getPost() != null ? appliance.getOffer().getPost().getName() : "";
                    ...
                    if (company != null && !company.trim().isEmpty()) {
                        String companyName = appliance.getOffer() != null ? appliance.getOffer().getCompanyName() : "";
                        if (companyName == null ||
